...
for simbolo in activos_validos:
    default_view = {"EMB": 5.5, "EWW": 8.0, "GNR": 5.0, "RXI": 7.0, "SHY": 2.0}.get(simbolo, media_rendimientos[simbolo] * 100)
    vistas[simbolo] = st.sidebar.number_input(f"Rendimiento esperado para {simbolo} (%):", value=default_view)

vistas_vector = np.array([vistas[s] / 100 for s in activos_validos])
P = np.eye(len(activos_validos))  # Matriz de identidad para views específicas
